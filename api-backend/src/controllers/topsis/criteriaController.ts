...
import {
  ...
  updateCriteria,
  deleteCriteria,
} from "../../models/topsis/criteriaModel";
...
export const getPagenitionCriteria = async (req: Request, res: Response) => {
  const page = parseInt(req.query.page as string, 10) || 1;
  const pageSize = parseInt(req.query.pageSize as string, 10) || 10;
  const offset = (page - 1) * pageSize;
  try {
    const { data, totalRows, totalPages } = await getPageCriteria(
      pageSize,
      offset
    );
    res.json({
...
export const addCriteria = async (req: Request, res: Response) => {
  ...
  try {
    const result = await createCreate(name, weight, attribute);
...
export const editCriteria = async (req: Request, res: Response) => {
  ...
  try {
    const result = await updateCriteria(parseInt(id), name, weight, attribute);
