...
const topsis = (alternatives, criteria, scores) => __awaiter(void 0, void 0, void 0, function* () {
    const nAlt = alternatives.length;
    const nCrit = criteria.length;
    // 1. Normalisasi bobot agar total = 1
    const totalWeight = criteria.reduce((sum, c) => sum + c.weight, 0);
    criteria = criteria.map((c) => (Object.assign(Object.assign({}, c), { weight: c.weight / totalWeight })));
    // 2. matriks keputusan
    const matrix = alternatives.map((alt) => {
        return criteria.map((crit) => {
            const score = scores.find((s) => s.alternative_id === alt.id && s.criteria_id === crit.id);
            // rumus untuk range
    ...
    const norm = Array.from({ length: nAlt }, () => []);
    for (let j = 0; j < nCrit; j++) {
        const col = matrix.map((row) => row[j]);
        const denom = Math.sqrt(col.reduce((sum, val) => sum + Math.pow(val, 2), 0));
    ...
    // 4. matriks terbobot
    const weighted = norm.map((row) => row.map((val, j) => val * criteria[j].weight));
    ...
    const results = alternatives.map((alt, i) => {
        const Dpos = Math.sqrt(weighted[i].reduce((sum, val, j) => sum + Math.pow(val - idealPos[j], 2), 0));
        const Dneg = Math.sqrt(weighted[i].reduce((sum, val, j) => sum + Math.pow(val - idealNeg[j], 2), 0));
    ...
    yield db_1.db.query(`DELETE FROM results_topsis`);
    // 9. simpan hasil ke database
    for (let i = 0; i < results.length; i++) {
        const r = results[i];
        const ranking = i + 1;
        yield db_1.db.query(`INSERT INTO results_topsis (alternative_id, total_score, ranking) VALUES (?, ?, ?)`, [r.id, r.score, ranking]);
    }
    return results;
});
